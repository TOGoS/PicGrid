package togos.picgrid.webserver;
		...
		public void run() {
			try {
				...
				if( res.getContent() instanceof byte[] ) {
					content = new SingleChunkByteBlob(new SimpleByteChunk((byte[])res.getContent()));
				} else if( res.getContent() instanceof String ) {
					content = new SingleChunkByteBlob(new SimpleByteChunk(((String)res.getContent()).getBytes("UTF-8")));
					if( contentType == null ) contentType = "text/plain";
					contentType += "; charset=utf-8";
				} else if( res.getContent() instanceof ByteBlob ) {
					content = (ByteBlob)res.getContent();
				...
				if( content != null ) {
					...
					if( len >= 0 ) {
						responseHeaders += "Content-Length: "+len+"\r\n";
					}
				}
				if( contentType != null ) {
				...
				if( content != null ) {
					for( Iterator i=content.chunkIterator(); i.hasNext(); ) {
						ByteChunk c = (ByteChunk)i.next();
						o.write(c.getBuffer(), c.getOffset(), c.getSize());
					}
				}
				o.flush();
